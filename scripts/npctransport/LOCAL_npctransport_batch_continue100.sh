...
for ID in ${IDs[@]}; do
  ...
  /cs/labs/ravehb/roi.eliasian/NpcTransportExperiment/HS-AFM-Simulation/scripts/npctransport/LOCAL_npctransport_sequential.sh 0 $START $STEP $OUTPUT_PATH/${ID} $CONFIG_PATH &
  PID=$!
  PIDs+=($PID)
